...
class GOOGLE_AUDIO(object):
    ...
    def __init__(self, config={}):
        ...
        self.__default(config, 'max_target_len', 4716) # overwrite cache
        self.__default(config, 'features', 'audio') # overwrite cache
        self.__default(config, 'encoding', '?') # overwrite cache
        ...
        # tfrecord path
        self.data_path = './data'
        self.train_file_paths = self.data_path + '/train/*.trecord'
        self.test_file_paths = self.data_path + '/test/*.trecord'
    ...
    def load(self):
        ...
        # sanity check
        if not num_train_files: print '... error, no train data files found in %s' % self.train_file_paths
        if not num_test_files: print '... error, no test data files found in %s' % self.test_file_paths
